...
(()=>{
  ...
  const serverBox=document.getElementById('server-military-extra');if(!serverBox)return;note.closest('label')?.after(serverBox);
...
(()=>{
  ...
  const matches=(r,p,y)=>r.type==='ANNUAL'&&(!r.object_type||r.object_type===p?.object_type)&&(r.min===null||r.min===undefined||r.min===''||r.min<=y)&&(r.max===null||r.max===undefined||r.max===''||r.max>=y);
   ...
   panel.querySelector('#military-locality')?.closest('label')?.remove();
...
<script>
(()=>{const form=document.getElementById('leave-proposal-form'),summary=document.getElementById('leave-summary-text');if(!form||!summary||!@json($isMilitaryAccount ?? false))return;const base=Number(@json($militaryAnnualDays ?? 0)),service=Number(@json($militaryServiceYears ?? 0)),note=document.getElementById('proposal-reason'),serverBox=document.getElementById('server-military-extra'),dynamicBox=document.getElementById('military-extra-standards');if(!serverBox)return;
