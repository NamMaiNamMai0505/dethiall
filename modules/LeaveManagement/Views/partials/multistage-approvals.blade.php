    ...
    @forelse($items as $pending)
        ...
        <div class="rounded-xl border {{ $isCommander?'border-amber-200 bg-amber-50/50':'border-blue-200 bg-blue-50/40' }} p-4">
            <div class="flex flex-wrap items-start justify-between gap-3">
                ...
                <span class="rounded-full px-3 py-1 text-xs font-bold {{ $isCommander?'bg-amber-100 text-amber-800':'bg-blue-100 text-blue-800' }}">{{ $isCommander?'Chờ chỉ huy cơ quan':'Chờ cơ quan quản lý' }}</span>
            </div>
            <div class="mt-3 flex flex-wrap items-center gap-2">
                <a href="{{ route('leave-management.requests.show',$pending) }}" class="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700">Xem chi tiết / in</a>
                @if($isCommander)
