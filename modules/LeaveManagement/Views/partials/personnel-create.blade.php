<form method="POST" action="{{ route('leave-management.personnel.store') }}" class="rounded-2xl border border-blue-100 bg-gradient-to-r from-blue-50 to-white p-5 shadow-sm">
    ...
    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="xl:col-span-2"><label class="mb-1.5 block text-sm font-bold text-slate-700">Đơn vị quân nhân</label><select name="unit_id" class="w-full rounded-xl border px-3 py-2.5"><option value="">Chọn đơn vị theo cây</option>@foreach($units as $unit)@php($level=max(1,(int)($unit->level ?: 1)))<option value="{{ $unit->id }}" data-level="{{ $level }}">{{ str_repeat('— ', max(0, $level - 1)) }}{{ $unit->code ? $unit->code.' — ' : '' }}{{ $unit->leafFirstHierarchyPath(' / ') }}</option>@endforeach</select></div>
        ...
        <div class="xl:col-span-2"><label class="mb-1.5 block text-sm font-bold text-slate-700">Quê quán</label><select name="hometown" class="w-full rounded-xl border px-3 py-2.5"><option value="">Chọn phường/xã và tỉnh/TP</option>@foreach(\Modules\LeaveManagement\Models\LeaveLocality::with('parent')->orderBy('level')->orderBy('name')->get() as $locality)@php($localityLabel=$locality->parent ? $locality->name.' — '.$locality->parent->name : $locality->name)<option value="{{ $localityLabel }}">{{ $localityLabel }}</option>@endforeach</select></div>
        <div class="xl:col-span-2"><label class="mb-1.5 block text-sm font-bold text-slate-700">Địa chỉ thường trú</label><select name="permanent_residence" class="w-full rounded-xl border px-3 py-2.5"><option value="">Chọn phường/xã và tỉnh/TP</option>@foreach(\Modules\LeaveManagement\Models\LeaveLocality::with('parent')->orderBy('level')->orderBy('name')->get() as $locality)@php($localityLabel=$locality->parent ? $locality->name.' — '.$locality->parent->name : $locality->name)<option value="{{ $localityLabel }}">{{ $localityLabel }}</option>@endforeach</select></div>
    </div>
