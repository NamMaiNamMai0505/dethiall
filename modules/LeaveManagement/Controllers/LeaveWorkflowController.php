...
class LeaveWorkflowController extends ModuleBaseController {
    ...
    public function reportWord(Request $request){$year=(int)($request->input('year')?:now()->year);$word=new \PhpOffice\PhpWord\PhpWord();$word->setDefaultFontName('Arial');$section=$word->addSection();$section->addTitle('BÁO CÁO NGHỈ PHÉP '.$year,1);$section->addText('Ngày lập: '.now()->format('d/m/Y H:i'));$table=$section->addTable(['borderSize'=>6,'borderColor'=>'999999']);$header=$table->addRow();foreach(['STT','Nhân sự','Loại','Từ ngày','Đến ngày','Số ngày','Trạng thái'] as $head)$header->addCell(1600)->addText($head);$rows=LeaveRequest::with('personnel')->whereYear('from_date',$year)->latest()->get();foreach($rows as $index=>$item){$row=$table->addRow();foreach([$index+1,$item->personnel?->name,$item->leave_type,$item->from_date?->format('d/m/Y'),$item->to_date?->format('d/m/Y'),$item->total_days,$item->status] as $value)$row->addCell(1600)->addText((string)($value??'—'));}if($rows->isEmpty())$table->addRow()->addCell(11200)->addText('Không có dữ liệu trong năm '.$year.'.');$path=storage_path('app/report-phep-'.now()->format('YmdHis').'.docx');(new \PhpOffice\PhpWord\Writer\Word2007($word))->save($path);return response()->download($path,'bao-cao-nghi-phep-'.$year.'.docx')->deleteFileAfterSend(true);}
    ...
    public function reportWordFixed(Request $request){
        ...
        $word=new \PhpOffice\PhpWord\PhpWord();$word->setDefaultFontName('Arial');$section=$word->addSection();
        $section->addTitle('BÁO CÁO NGHỈ PHÉP '.$year,1);$section->addText('Ngày lập: '.now()->format('d/m/Y H:i'));
        $table=$section->addTable(['borderSize'=>6,'borderColor'=>'999999']);$header=$table->addRow();
        foreach(['STT','Nhân sự','Loại','Từ ngày','Đến ngày','Số ngày','Trạng thái'] as $head)$header->addCell(1600)->addText($head);
        $rows=LeaveRequest::with('personnel')->whereYear('from_date',$year)->latest()->get();
        foreach($rows as $index=>$item){$row=$table->addRow();foreach([$index+1,$item->personnel?->name,$item->leave_type,$item->from_date?->format('d/m/Y'),$item->to_date?->format('d/m/Y'),$item->total_days,$item->status] as $value)$row->addCell(1600)->addText((string)($value??'—'));}
        if($rows->isEmpty())$table->addRow()->addCell(11200)->addText('Không có dữ liệu trong năm '.$year.'.');
