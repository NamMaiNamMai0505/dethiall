...
@section('page-title', 'Chi tiết phòng học')
...
<x-page-header
    ...
    :actions="[
        ...
        [
            'url' => route('classrooms.edit', $classroom),
            'label' => 'Chỉnh sửa',
            'icon' => 'pencil',
            'color' => 'blue'
        ]
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-900">{{ $classroom->id }}</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Tên phòng học</label>
                    <p class="text-sm text-gray-900 font-semibold">{{ $classroom->name }}</p>
                </div>
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-900">{{ $classroom->creator->name ?? 'N/A' }}</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Ngày tạo</label>
                    <p class="text-sm text-gray-900">{{ $classroom->created_at->format('d/m/Y H:i:s') }}</p>
                </div>

                @if($classroom->updated_by)
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Người cập nhật</label>
                    <p class="text-sm text-gray-900">{{ $classroom->updater->name ?? 'N/A' }}</p>
                </div>
                @endif

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Ngày cập nhật</label>
                    <p class="text-sm text-gray-900">{{ $classroom->updated_at->format('d/m/Y H:i:s') }}</p>
                </div>
            </div>
        </div>
