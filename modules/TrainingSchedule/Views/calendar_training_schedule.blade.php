        ...
        <thead>
            <tr class="bg-gradient-to-r from-blue-600 to-blue-700">
                ...
                @foreach($periods as $day)
                    ...
                    @endphp
                    <th class="px-3 py-3 border-r border-blue-500 last:border-r-0 min-w-[180px] sticky top-0 {{ $isToday ? 'bg-yellow-500 z-10' : 'bg-blue-700 z-10' }}">
                        <div class="text-center">
                            <div class="{{ $isToday ? 'text-gray-900' : 'text-white' }} font-bold text-base">
                                {{ \Carbon\Carbon::parse($day['date'])->format('d/m') }}
                            </div>
                            <div class="{{ $isToday ? 'text-gray-800' : 'text-blue-100' }} text-xs mt-1 font-medium">
                                {{ $day['weekday'] }}
                            </div>
                            @if($isToday)
        ...
        <tbody>
            @for($period = 1; $period <= 9; $period++)
                <tr class="hover:bg-blue-50/50 transition-colors duration-150 {{ $period % 2 == 0 ? 'bg-gray-50/50' : 'bg-white' }}">
                    ...
                    @foreach($periods as $day)
                        @php 
                            $detail = $day['periods'][$period] ?? null; 
                            $isToday = \Carbon\Carbon::parse($day['date'])->isToday();
                        @endphp
                        <td class="border px-3 py-4 align-top {{ $isToday ? 'bg-yellow-50/30' : '' }}">
