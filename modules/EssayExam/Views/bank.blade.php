...
@foreach($specializationExams as $exam) @foreach($exam->questions->where('paper_status','APPROVED')->groupBy('paper_number') as $paper=>$questions) @php($qrValue=($exam->approval_qr ?: 'QR-EXAM-'.$exam->id).'-D'.str_pad((string) $paper,2,'0',STR_PAD_LEFT))<tr><td class="p-3 font-semibold">{{ $exam->paperCode((int) $paper) }}</td><td class="p-3">{{ $exam->subject->name ?? '—' }}</td><td class="p-3 text-center">{{ $exam->academic_year ?? '—' }}</td><td class="p-3 text-center">{{ $semesterLabel($exam->semester) }}</td><td class="p-3 text-center">{{ $difficultyLabel($exam->difficulty) }}</td><td class="p-3 text-center">{{ $examTypeLabel($exam->exam_type) }}</td><td class="p-3 text-center font-bold">{{ $paper }}</td><td class="p-3 text-center"><img class="w-20 h-20 mx-auto border rounded" alt="QR khóa đề" src="https://api.qrserver.com/v1/create-qr-code/?size=120x120&data={{ urlencode($qrValue) }}"></td><td class="p-3 text-center"><span class="px-2 py-1 rounded bg-emerald-100 text-emerald-800 text-xs">Đã khóa</span></td></tr>@endforeach @endforeach
