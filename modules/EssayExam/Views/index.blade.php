...
<form id="essay-list-filter" method="GET" action="{{ route($mine ? 'essay-exams.mine' : 'essay-exams.index') }}" data-turbo="false" class="bg-white border rounded-lg p-4 mb-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
    ...
    <select name="specialization_id" id="essay-filter-specialization" data-native-select="1" class="border rounded-lg px-3 py-2"><option value="">Tất cả ngành</option>@foreach($specializations as $specialization)<option value="{{ $specialization->id }}" data-system="{{ $specialization->training_system_id }}" @selected(request('specialization_id') == $specialization->id)>{{ $specialization->selection_label }}</option>@endforeach</select>
