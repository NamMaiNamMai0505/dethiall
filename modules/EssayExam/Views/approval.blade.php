...
<section id="bgh-signature-panel" class="bg-white border border-amber-200 rounded-xl p-4 mb-4">
    ...
    <div class="flex flex-wrap items-center gap-2 mt-3">
        <select id="bgh-print-target" class="border rounded-lg px-3 py-2 text-sm min-w-[260px]"><option value="">Chọn bộ đề cần in</option>@foreach($exams as $exam)<option value="{{ $loop->index }}" data-pdf="{{ $exam->source_pdf_path ? asset('storage/'.$exam->source_pdf_path) : '' }}">{{ $exam->code }} — {{ $exam->title }}</option>@endforeach</select>
