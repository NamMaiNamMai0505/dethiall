...
class UpdateSubjectRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'code' => [
                ...
                'max:50',
                'alpha_dash',
                Rule::unique('subjects')->ignore($subjectId)->whereNull('deleted_at'),
            ],
            'abbreviation' => [
                'nullable',
                'string',
    ...
    protected function prepareForValidation(): void
    {
        // Set default values
        $this->merge([
            'is_required' => $this->has('is_required') ? (bool) $this->is_required : $this->route('subject')->is_required,
            'is_active' => $this->has('is_active') ? (bool) $this->is_active : $this->route('subject')->is_active,
        ]);
        ...
        if ($this->has('semester') && ! empty($this->semester)) {
            ...
            if (preg_match('/^[1-6]$/', $sem)) {
                ...
                $sem = $this->route('subject')->semester ?? 'semester_1';
            }
            $this->merge(['semester' => $sem]);
        } else {
            $this->merge(['semester' => $this->route('subject')->semester ?? 'semester_1']);
        }
        ...
        $name = trim((string) $this->input('name', $this->route('subject')->name ?? ''));
