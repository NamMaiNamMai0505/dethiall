...
class HourNorm extends Model
{
    use HasStandardHoursPeriod, SoftDeletes;

    protected $table = 'standard_hour_norms';
    ...
    {
        $ratio = (float) ($this->position?->ratio_percent ?? 100);

        return round((float) $this->standard_hours * $ratio / 100, 2);
    }

    public function getMinClassroomHoursAttribute(): float
    {
        $minPercent = (float) ($this->position?->min_classroom_percent ?? 50);

        return round($this->effective_hours * $minPercent / 100, 2);
    }
