...
@section('page-title', 'Kê khai giờ chuẩn hằng năm')
...
@php
    ...
    $isManagementDeclaration = !auth()->user()?->isInstructor();
    $instructorParams = $isManagementDeclaration ? ['instructor_id' => $instructor->id] : [];
    $backParams = array_merge(['year' => $defaultYear], $instructorParams);
...
<x-breadcrumb :items="[
    ...
    ['title' => 'Kê khai giờ chuẩn', 'url' => route('standard-hours.my-results.index', $instructorParams)],
    ['title' => 'Hồ sơ '.$defaultPeriodLabel]
]" />

<x-page-header title="HỒ SƠ KÊ KHAI GIỜ CHUẨN {{ mb_strtoupper($periodModeLabel) }} {{ $defaultPeriodLabel }}" :actions="[
    ['url' => route('standard-hours.my-results.index', $backParams), 'label' => 'Quay lại', 'icon' => 'arrow-left', 'color' => 'gray'],
]" />
...
<form action="{{ route('standard-hours.my-results.store-declaration') }}" method="POST" data-turbo="false">
    @csrf
    @if($isManagementDeclaration)
    ...
    <div class="mb-6 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-200 bg-gradient-to-r from-slate-900 to-blue-900 px-5 py-4 text-white">
            <h2 class="text-lg font-semibold">Thông tin giảng viên kê khai</h2>
            <p class="mt-1 text-sm text-blue-100">
                {{ $isManagementDeclaration
                    ? 'Admin đang lập/cập nhật hồ sơ cho giảng viên đã chọn; mọi thay đổi vẫn được lưu vết người thao tác.'
                    : 'Thông tin được lấy tự động từ tài khoản đang đăng nhập.' }}
            </p>
        </div>
        <div class="grid grid-cols-1 gap-4 p-5 sm:grid-cols-2 lg:grid-cols-4">
            <div>
                ...
                <div class="mt-1 text-lg font-bold text-blue-800">{{ $instructor->code }}</div>
            </div>
            <div>
    ...
    <div class="mb-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div class="mb-5 flex items-center gap-2">
            <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-blue-100 text-blue-700"><i class="bi bi-calendar-range"></i></span>
            <div>
                <h2 class="font-semibold text-slate-900">Thời gian và định mức áp dụng</h2>
                <p class="text-sm text-slate-500">Mỗi hồ sơ chỉ nằm trong một {{ mb_strtolower($periodModeLabel) }} theo cấu hình Giờ chuẩn GV.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
                <label class="mb-2 block font-medium text-slate-800" for="object_type_id">Đối tượng <span class="text-red-500">*</span></label>
                <div class="ui-select-field">
                    <select name="object_type_id" id="object_type_id" data-placeholder="Chọn đối tượng" data-searchable="1" class="w-full" required>
                ...
                @error('object_type_id')<span class="mt-1 block text-sm text-red-600">{{ $message }}</span>@enderror
            </div>

            <div>
                <label class="mb-2 block font-medium text-slate-800" for="position_id">Chức danh <span class="text-red-500">*</span></label>
                <div class="ui-select-field">
                    <select name="position_id" id="position_id" data-placeholder="Chọn chức danh" data-searchable="1" class="w-full" required>
                ...
                @error('position_id')<span class="mt-1 block text-sm text-red-600">{{ $message }}</span>@enderror
            </div>
        </div>

        <div class="mt-5 grid grid-cols-1 gap-3 rounded-xl border border-blue-100 bg-blue-50 p-4 md:grid-cols-4">
            <div>
            ...
            <div>
                <span class="text-xs uppercase tracking-wide text-blue-700">Số giờ chuẩn quy định</span>
                <div class="mt-1 text-2xl font-bold text-blue-800"><span id="effective-norm-hours">—</span> giờ</div>
            </div>
        </div>
    </div>
    ...
    <div class="mb-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500">
                    Chọn khoảng ngày tra cứu tự do, kể cả qua nhiều năm/năm học. Khoảng này không thay đổi kỳ dùng để lưu hồ sơ.
                </p>
            </div>
        ...
        <div id="retrieve-error" class="mt-4 hidden rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700"></div>
        ...
        <div id="empty-schedule-state" class="mt-5 hidden rounded-lg border border-dashed border-slate-300 px-4 py-8 text-center text-sm text-slate-500">
        ...
        <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
            <div>
                ...
                <div class="flex items-center gap-2">
                    <input type="number" id="other_teaching_hours" name="other_teaching_hours" min="0" max="100000" step="0.01"
                           value="{{ $otherHours }}" class="form-input w-full" {{ $isLocked ? 'readonly' : '' }} required>
                    <span class="font-medium text-slate-600">giờ</span>
                </div>
                @error('other_teaching_hours')<span class="mt-1 block text-sm text-red-600">{{ $message }}</span>@enderror
            </div>
            <div class="lg:col-span-2">
                ...
                <textarea id="other_teaching_notes" name="other_teaching_notes" rows="3"
                          ...
                          class="form-textarea w-full" {{ $isLocked ? 'readonly' : '' }}>{{ old('other_teaching_notes', $declaration->other_teaching_notes ?? '') }}</textarea>
                @error('other_teaching_notes')<span class="mt-1 block text-sm text-red-600">{{ $message }}</span>@enderror
            </div>
        </div>
    </div>

    <div class="mb-6 rounded-xl border border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50 p-5 shadow-sm">
        <h2 class="mb-4 font-semibold text-slate-900">Kết quả kê khai tạm tính</h2>
        <div class="grid grid-cols-2 gap-3 lg:grid-cols-5">
            <div class="rounded-lg bg-white p-3"><p class="text-xs text-slate-500">Từ lịch</p><p class="mt-1 text-xl font-bold text-indigo-700"><span id="result-schedule-hours">0</span></p></div>
            <div class="rounded-lg bg-white p-3"><p class="text-xs text-slate-500">Giảng khác</p><p class="mt-1 text-xl font-bold text-amber-700"><span id="result-other-hours">0</span></p></div>
            <div class="rounded-lg bg-white p-3"><p class="text-xs text-slate-500">Tổng trực tiếp</p><p class="mt-1 text-xl font-bold text-blue-800"><span id="result-direct-hours">0</span></p></div>
            <div class="rounded-lg bg-white p-3"><p class="text-xs text-slate-500">HĐCM quy đổi</p><p class="mt-1 text-xl font-bold text-emerald-700"><span id="result-conversion-hours">0</span></p></div>
            <div class="col-span-2 rounded-lg bg-blue-800 p-3 text-white lg:col-span-1"><p class="text-xs text-blue-100">Tổng giờ chuẩn</p><p class="mt-1 text-2xl font-bold"><span id="result-total-hours">0</span></p></div>
        </div>
    ...
    <div class="flex justify-end gap-3">
        <a href="{{ route('standard-hours.my-results.index', $backParams) }}" class="{{ \Modules\StandardHours\Support\ActionButton::classes('secondary') }}">Hủy</a>
        @unless($isLocked)
...
(function () {
    ...
    const endpoint = @json(route('standard-hours.my-results.retrieve-schedule'));
    const instructorId = @json($isManagementDeclaration ? $instructor->id : null);
    ...
    let conversionHours = Number(initialSnapshot.conversion_hours || @json((float) ($declaration->conversion_hours ?? 0)));
