...
<div class="mb-6 rounded-xl border border-indigo-100 bg-gradient-to-r from-indigo-50 to-blue-50 p-4">
    <div class="flex gap-3">
        <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-indigo-600 text-white">
...
<x-form.instructor-select
    name="instructor_id"
    label="Giảng viên kê khai"
    :instructors="$instructors"
    :value="old('instructor_id', $researchRecord->instructor_id ?? $scopedInstructorId)"
    :required="true"
...
<div class="mb-6">
    <label class="mb-2 block font-medium text-slate-800" for="research_category_id">
        Loại sản phẩm/hoạt động NCKH <span class="text-red-500">*</span>
    </label>
    <div class="ui-select-field">
        <select name="research_category_id" id="research_category_id"
                data-placeholder="Chọn loại sản phẩm"
                data-searchable="1"
                class="w-full @error('research_category_id') border-red-500 @enderror" required>
    ...
    @error('research_category_id')<span class="mt-1 block text-sm text-red-600">{{ $message }}</span>@enderror
</div>

<div class="mb-6">
    <label class="mb-2 block font-medium text-slate-800" for="product_name">
        Tên/chi tiết sản phẩm <span class="text-red-500">*</span>
    </label>
    <input type="text" id="product_name" name="product_name"
           value="{{ old('product_name', $researchRecord->product_name ?? '') }}"
           placeholder="Ví dụ: Giáo trình môn..., bài báo đăng tại..."
           class="form-input w-full @error('product_name') border-red-500 @enderror" required>
    @error('product_name')<span class="mt-1 block text-sm text-red-600">{{ $message }}</span>@enderror
</div>
...
<div class="mb-6 grid grid-cols-1 gap-5 md:grid-cols-3">
    ...
    <div>
        ...
        <input type="text" id="publication_place" name="publication_place"
               ...
               class="form-input w-full @error('publication_place') border-red-500 @enderror">
        @error('publication_place')<span class="mt-1 block text-sm text-red-600">{{ $message }}</span>@enderror
    </div>
</div>
    ...
    <div class="grid grid-cols-1 gap-5 md:grid-cols-3">
        <div>
            <label class="mb-2 block font-medium text-slate-800" for="member_count">
                Tổng số thành viên tham gia <span class="text-red-500">*</span>
            </label>
            <input type="number" id="member_count" name="member_count" min="1" max="99" step="1"
                   value="{{ $memberCount }}"
                   class="form-input w-full @error('member_count') border-red-500 @enderror" required>
            <p class="mt-1 text-xs text-slate-500">Mặc định là 1, có thể sửa theo sản phẩm thực tế.</p>
            @error('member_count')<span class="mt-1 block text-sm text-red-600">{{ $message }}</span>@enderror
        </div>

        <div>
            <label class="mb-2 block font-medium text-slate-800" for="participation_type">
                Vai trò/chức danh của bạn <span class="text-red-500">*</span>
            </label>
            <div class="ui-select-field">
                <select name="participation_type" id="participation_type"
                        data-searchable="0" class="w-full" required>
            ...
            @error('participation_type')<span class="mt-1 block text-sm text-red-600">{{ $message }}</span>@enderror
        </div>

        <div>
            <label class="mb-2 block font-medium text-slate-800" for="duration_years">
                Số năm thực hiện <span class="text-red-500">*</span>
            </label>
            <input type="number" id="duration_years" name="duration_years" min="1" max="20" step="1"
                   value="{{ $durationYears }}"
                   class="form-input w-full @error('duration_years') border-red-500 @enderror" required>
            <p class="mt-1 text-xs text-slate-500">Giờ sản phẩm được chia đều cho số năm thực hiện.</p>
            @error('duration_years')<span class="mt-1 block text-sm text-red-600">{{ $message }}</span>@enderror
        </div>
    </div>
    ...
    <div id="contribution-field" class="mt-5 {{ $participation === \Modules\StandardHours\Models\ResearchRecord::PARTICIPATION_MEMBER ? '' : 'hidden' }}">
        ...
        <div class="relative max-w-xs">
            <input type="number" id="contribution_percent" name="contribution_percent"
                   ...
                   class="form-input w-full pr-10 @error('contribution_percent') border-red-500 @enderror">
        ...
        @error('contribution_percent')<span class="mt-1 block text-sm text-red-600">{{ $message }}</span>@enderror
    </div>
</div>

<div id="research-hours-preview" class="mb-6 rounded-xl border border-blue-200 bg-blue-50 p-5">
    <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
        <div>
            <p class="text-xs font-semibold uppercase tracking-wide text-blue-700">Hệ thống tự tính theo Luật quy đổi</p>
    ...
    <div class="mt-4 grid grid-cols-1 gap-2 text-sm sm:grid-cols-3">
        <div class="rounded-lg border border-blue-100 bg-white/80 px-3 py-2">
...
<div class="mb-6 rounded-xl border border-amber-200 bg-amber-50 p-4">
    <label class="mb-2 block font-semibold text-slate-900" for="converted_hours">
        Số giờ bạn kê khai <span class="text-red-500">*</span>
    </label>
    <div class="flex max-w-sm items-center gap-2">
        <input type="number" id="converted_hours" name="converted_hours" min="0" max="1000000" step="0.01"
               ...
               class="form-input w-full @error('converted_hours') border-red-500 @enderror"
    ...
    @error('converted_hours')<span class="mt-1 block text-sm text-red-600">{{ $message }}</span>@enderror
</div>
...
<div class="mb-6">
    ...
    <textarea id="hours_adjustment_note" name="hours_adjustment_note" rows="2"
              ...
              class="form-textarea w-full @error('hours_adjustment_note') border-red-500 @enderror">{{ old('hours_adjustment_note', $researchRecord->hours_adjustment_note ?? '') }}</textarea>
    @error('hours_adjustment_note')<span class="mt-1 block text-sm text-red-600">{{ $message }}</span>@enderror
</div>

<div class="mb-6">
    <label class="mb-2 block font-medium text-slate-800" for="notes">Ghi chú chung</label>
    <textarea id="notes" name="notes" rows="3" class="form-textarea w-full">{{ old('notes', $researchRecord->notes ?? '') }}</textarea>
</div>

<div class="mb-2">
    <label class="mb-2 block font-medium text-slate-800" for="evidence">Minh chứng</label>
    @if(!empty($researchRecord?->evidence_path))
        <p class="mb-2 text-sm text-slate-600">
            File hiện tại:
            <a href="{{ $researchRecord->evidence_url }}" target="_blank" class="font-medium text-blue-600 hover:underline">Xem minh chứng</a>
        </p>
    @endif
    ...
    <p class="mt-1 text-sm text-slate-500">PDF, ảnh hoặc Word. Tối đa 5MB.</p>
    @error('evidence')<span class="mt-1 block text-sm text-red-600">{{ $message }}</span>@enderror
</div>
...
(function () {
    ...
    const normCard = document.getElementById('research-norm-card');
    ...
    function selectedInstructorId() {
        if (!instructorSelect) return '';
        if (typeof window.getSelectValue === 'function') {
            return String(window.getSelectValue(instructorSelect) || '');
        }
        return String(instructorSelect.value || '');
    }
    ...
    function updateResearchNorm() {
        if (!normCard) return;
        const norm = researchNorms?.[selectedInstructorId()] || null;
        ...
        if (normObject) {
            normObject.textContent = hasNorm
                ...
                : (selectedInstructorId() ? 'Chưa gắn đối tượng giờ chuẩn' : 'Chọn giảng viên để xem định mức');
        }
        if (normPeriod) {
    ...
    instructorSelect?.addEventListener('change', updateResearchNorm);
