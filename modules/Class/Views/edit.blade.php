    ...
    <form action="{{ route('classes.update', $class) }}" method="POST">
                        ...
                        <div class="ui-select-field">
                            <select id="training_system_id" name="training_system_id" required
                                ...
                                @foreach($trainingSystems ?? [] as $id => $name)
                                    <option value="{{ $id }}" @selected(old('training_system_id', $class->specialization?->training_system_id) == $id)>{{ $name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div>
                        ...
                        <div class="ui-select-field">
                            <select id="specialization_id" name="specialization_id" required
                                    data-searchable="1" data-placeholder="Chọn ngành...">
                                <option value="">Chọn ngành đào tạo...</option>
                                @foreach($specializations as $spec)
                                    <option value="{{ $spec->id }}"
                                            data-system="{{ $spec->training_system_id }}"
                                            @selected(old('specialization_id', $class->specialization_id) == $spec->id)>
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div>
                        <x-form.input
                            ...
                            required
                            min="1"
                            :value="old('duration_months', $class->duration_months)"
                        />
                    </div>
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div>
                        <x-form.input
                            ...
                            :value="old('management_unit', $class->management_unit)"
                        />
                    </div>
                    ...
                    <div>
                        <x-form.select
                            ...
                            :value="old('classroom_id', $class->classroom_id)"
                        />
                    </div>
                </div>
            </div>
        ...
        <div class="bg-gray-50 px-6 py-4 border-t flex justify-end space-x-2">
            ...
            <button type="submit" class="btn btn-primary">
                <i class="bi bi-save mr-2"></i>Cập nhật lớp học
            </button>
        </div>
    </form>
</div>
...
@include('partials.cascade-training-system', [
    ...
    'initialSystemId' => old('training_system_id', $class->specialization?->training_system_id),
    'initialSpecId' => old('specialization_id', $class->specialization_id),
])
@endpush
