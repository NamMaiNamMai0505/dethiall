<div class="space-y-5">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded',()=>{const type=document.getElementById('inventory-update-type'),reason=document.getElementById('inventory-update-reason');if(!type||!reason)return;const sync=()=>{const current=reason.value;const items=window.inventoryReasonOptions[type.value]||[];reason.innerHTML='<option value="">Chọn lý do</option>'+items.map(item=>`<option value="${item[0]}">${item[0]} - ${item[1]}</option>`).join('');if(items.some(item=>item[0]===current))reason.value=current;};type.addEventListener('change',sync);sync();});
        document.addEventListener('DOMContentLoaded',()=>{const form=document.querySelector('#inventory-panel-bulk form');if(!form)return;form.addEventListener('submit',()=>{const ensure=(name,value)=>{let field=form.querySelector(`input[name="${name}"]`);if(!field){field=document.createElement('input');field.type='hidden';field.name=name;form.append(field)}field.value=value||''};ensure('update_type',document.getElementById('inventory-update-type')?.value||'IN');const choice=document.getElementById('inventory-update-reason');ensure('reason_code',choice?.value||'');ensure('reason',window.inventoryReasonText(choice?.value||''));});});
    </script>
    <script>
    ...
    <div class="rounded-2xl border border-blue-100 bg-blue-50/60 px-5 py-4">
        ...
        <div id="inventory-update-meta" class="mt-4 grid gap-3 md:grid-cols-3">
            ...
            <label class="text-sm font-semibold text-slate-700">Lý do tăng / giảm <span class="text-rose-500">*</span><select id="inventory-update-reason" required class="mt-1 w-full rounded-lg border bg-white px-3 py-2.5"></select></label>
    ...
    <script>
     ...
     <script>
         document.addEventListener('turbo:load',()=>{
            ...
            if(bulk){bulk.addEventListener('submit',()=>{[['update_type','inventory-update-type'],['reason_code','inventory-update-reason'],['floor','inventory-update-floor']].forEach(([name,id])=>{let input=bulk.querySelector(`[data-workbench-field="${name}"]`);if(!input){input=document.createElement('input');input.type='hidden';input.dataset.workbenchField=name;input.name=name;bulk.append(input)}input.value=document.getElementById(id)?.value||''});let reason=bulk.querySelector('[data-workbench-field="reason"]');if(!reason){reason=document.createElement('input');reason.type='hidden';reason.dataset.workbenchField='reason';reason.name='reason';bulk.append(reason)}reason.value=window.inventoryReasonText?.(document.getElementById('inventory-update-reason')?.value||'')||'';})}
    ...
    <script>
        document.addEventListener('DOMContentLoaded',()=>{
            ...
            if(bulkType&&bulkReason){bulkForm?.addEventListener('submit',()=>{let code=bulkForm.querySelector('input[name="reason_code"]');if(!code){code=document.createElement('input');code.type='hidden';code.name='reason_code';bulkForm.append(code)}code.value=bulkReason.value||'';let reason=bulkForm.querySelector('input[data-workbench-field="reason"]');if(!reason){reason=document.createElement('input');reason.type='hidden';reason.name='reason';reason.dataset.workbenchField='reason';bulkForm.append(reason)}reason.value=window.inventoryReasonText?.(bulkReason.value||'')||bulkReason.value||'';});}
