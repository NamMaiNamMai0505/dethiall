...
class InventoryWorkflowController extends ModuleBaseController
{
    ...
    public function transferDecide(Request $r, InventoryTransfer $transfer)
    {
        ...
        DB::transaction(function () use ($d, $transfer, $r, $wasCompleted) {
            ...
            if ($transfer->asset) {
                ...
                if ($quantity === $current) {
                    $asset->update(['classroom_id' => $destination]);
                } else {
                    $asset->decrement('quantity', $quantity);
                    if ($destination !== null) {
                        $newAsset = $asset->replicate();
                        $newAsset->asset_code = $asset->asset_code . '-D' . now()->format('His') . random_int(10, 99);
            ...
            if ($transfer->material && !$transfer->asset) {
                $transfer->material->update(['classroom_id' => $destination]);
                $warehouseItemData = [
    ...
    public function roomAssetStore(Request $r,Classroom $classroom){$d=$r->validate(['material_id'=>'required|exists:inventory_materials,id','quantity'=>'required|numeric|min:.01']);$material=InventoryMaterial::with('category')->findOrFail($d['material_id']);abort_unless($classroom->managing_unit_id,'Phòng chưa có đơn vị quản lý.');$d['holding_unit_id']=$classroom->managing_unit_id;$asset=InventoryAsset::updateOrCreate(['material_id'=>$material->id,'classroom_id'=>$classroom->id],['asset_code'=>$material->code.'-P'.$classroom->id,'name'=>$material->name,'quantity'=>$d['quantity'],'unit'=>$material->unit,'holding_unit_id'=>$d['holding_unit_id'],'grade'=>1,'status'=>'NORMAL']);return back()->with('success','Đã thêm vật tư có sẵn vào phòng.');}
    public function roomAssetImport(Request $r,Classroom $classroom){$d=$r->validate(['file'=>'required|file|mimes:xlsx,xls,csv,txt|max:20480']);abort_unless($classroom->managing_unit_id,'Phòng chưa có đơn vị quản lý.');$d['holding_unit_id']=$classroom->managing_unit_id;$rows=\PhpOffice\PhpSpreadsheet\IOFactory::load($r->file('file')->getRealPath())->getActiveSheet()->toArray(null,true,true,true);$headers=array_map(fn($v)=>strtolower(trim((string)$v)),array_shift($rows));$count=0;foreach($rows as $row){$values=[];foreach($headers as $key=>$header)$values[$header]=trim((string)($row[$key]??''));if(empty($values['code']))continue;$material=InventoryMaterial::where('code',$values['code'])->first();if(!$material)continue;InventoryAsset::updateOrCreate(['material_id'=>$material->id,'classroom_id'=>$classroom->id],['asset_code'=>$material->code.'-P'.$classroom->id,'name'=>$material->name,'quantity'=>(float)($values['quantity']?:1),'unit'=>$material->unit,'holding_unit_id'=>$d['holding_unit_id'],'grade'=>1,'status'=>'NORMAL']);$count++;}return back()->with('success',"Đã import {$count} vật tư có sẵn vào phòng.");}
    public function roomAssetUpdate(Request $r,Classroom $classroom,InventoryAsset $asset){abort_unless((int)$asset->classroom_id===(int)$classroom->id,404);abort_unless($classroom->managing_unit_id,'Phòng chưa có đơn vị quản lý.');$d=$r->validate(['quantity'=>'required|numeric|min:.01','grade'=>'required|integer|min:1|max:5','status'=>'required|in:NORMAL,BROKEN,REPAIRING,LIQUIDATED']);$d['holding_unit_id']=$classroom->managing_unit_id;$asset->update($d);return back()->with('success','Đã sửa vật tư trong phòng.');}
    public function roomAssetDelete(Classroom $classroom,InventoryAsset $asset){abort_unless((int)$asset->classroom_id===(int)$classroom->id,404);abort_if($asset->repairs()->exists()||$asset->proposals()->exists(),422,'Không thể xóa vật tư đã có lịch sử.');$asset->delete();return back()->with('success','Đã xóa vật tư khỏi phòng.');}
