...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="lms-card rounded-2xl overflow-hidden">
        <div class="bg-gradient-to-r from-teal-600 to-teal-500 px-6 py-5 text-white flex flex-wrap items-center justify-between gap-3">
            <h1 class="text-lg font-bold"><i class="bi bi-pencil-square mr-2"></i>Chỉnh sửa kê khai</h1>
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('lms.teach.standard-hours.conversion-records.show', $conversionRecord) }}" class="px-4 py-2.5 rounded-xl border border-white/30 text-white hover:bg-white/10 text-sm font-medium transition-colors">
    ...
    <div class="lms-card rounded-2xl p-5">
        <form action="{{ route('lms.teach.standard-hours.conversion-records.update', $conversionRecord) }}" method="POST"
              enctype="multipart/form-data" data-conversion-record-form data-turbo="false">
            @csrf @method('PUT')
            @include('lms::teach.standard-hours.conversion-records._form', ['conversionRecord' => $conversionRecord])
