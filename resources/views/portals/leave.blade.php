...
<div class="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-4">
    ...
    <a href="{{ route('leave-management.reports', ['report_tab' => 'notifications']) }}" class="rounded-xl border border-amber-100 border-l-4 border-l-amber-500 bg-white p-5 shadow-sm"><p class="text-sm text-slate-600">Thông báo chưa đọc</p><p class="mt-1 text-3xl font-bold text-amber-700">{{ number_format($stats['unread_alerts'] ?? 0) }}</p></a>
</div>
